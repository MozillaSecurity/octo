...
class css extends make {
   ...
   */
  static angle () {
    let [suffix, limit] = random.choose([
      ['deg', 360],
      ['grad', 400],
      ['rad', Math.PI * 2],
      ['turn', 1]
   ...
   */
  static decibel () {
    return `${make.number.frange(0, make.number.any())}dB`
  }
   ...
   */
  static number () {
    return (random.chance(10) ? make.number.any() : make.number.frange(0, make.number.any()))
  }
  ...
  static numberOneOrGreater () {
    return make.number.frange(0, Math.abs(make.number.any()))
  }
